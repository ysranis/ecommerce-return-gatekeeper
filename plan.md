...
- [x] `data/bitext_seeds.jsonl` — 2,000 filtered seed rows
- [x] `data/distilled_dataset.jsonl` — ≥1,500 validated golden rows
- [x] `data/train.jsonl` (1,200 rows), `data/val.jsonl` (150 rows), `data/test.jsonl` (150 rows, held out)
- [x] **Quality Gate 1 passed:** ≥1,500 validated rows confirmed

### Week 2 — ✅ Complete

- [x] `results/baseline_results_qwen.json` — zero-shot Qwen baseline (150 rows)
- [x] `results/baseline_results_llama.json` — zero-shot Llama baseline (150 rows)
- [x] `results/finetuned_results_qwen.json` — fine-tuned Qwen evaluation (150 rows)
- [x] `results/finetuned_results_llama.json` — fine-tuned Llama evaluation (150 rows)
- [x] `output/qwen-2.5-7b-ecommerce-gk/` — fine-tuned Qwen adapter weights (on RunPod, not uploaded to HF Hub yet)
- [x] `output/llama-3.2-3b-ecommerce-gk/` — fine-tuned Llama adapter weights (on RunPod, not uploaded to HF Hub yet)
- [x] `router/triage_router.py` — triage router prototype
- [x] W&B training run links for both models
- [x] **Quality Gate 2 passed:** Training completed on RunPod RTX 4090, loss curves stable

> Note: MMLU guard metric not run (deprioritised — port to second training run).

### Week 3 — ✅ Complete

- [x] Vercel Postgres (Neon) populated with all eval results
- [x] `results/eval_results.json` — full 5-model × 150-row evaluation
- [x] `results/eval_summary.json` — aggregated metrics with bootstrap 95% CIs
- [x] LLM-as-a-Judge scores per row (DeepSeek-V3 judge, ~750 calls, ~$0.11)
- [x] `results/comparison_report.json` + `results/comparison_report.md`
- [x] `results/cross_comparison_table.md` — portfolio delta table

> **Quality Gate 3 status:** Intent accuracy targets met (FT Qwen 86.0%, FT Llama 74.0%). **Gatekeeper accuracy targets not met** (62.0% vs ≥97% for Track A; 58.0% vs ≥92% for Track B). JSON validity targets met. This drives the planned second training run.

### Actual Results — Run 1

| Model | JSON Valid | Intent Acc. | GK Acc. | Slot F1 | Halluc. | Judge |
|---|---|---|---|---|---|---|
| Base Qwen-2.5-7B | 99.3% | 61.3% | 33.3% | 91.8% | 2.0% | 3.08/5 |
| **FT Qwen-2.5-7B (A)** | 96.7% | **86.0%** | **62.0%** | 92.0% | 2.7% | **3.94/5** |
| Base Llama-3.2-3B | 94.7% | 24.0% | 26.0% | 86.3% | 4.7% | 2.39/5 |
| **FT Llama-3.2-3B (B)** | **98.0%** | **74.0%** | **58.0%** | **91.6%** | **0.7%** | **3.69/5** |
| DeepSeek-V3 (teacher) | 89.3% | 81.3% | 73.3% | 85.8% | 2.0% | 4.07/5 |

Fine-tuning lifted Qwen intent accuracy by **+24.7pp** and Llama by **+50.0pp**. Gatekeeper accuracy improved by **+28.7pp** (Qwen) and **+32.0pp** (Llama) but did not reach PRD targets — second run planned.

### Week 4 — ✅ Complete

- [x] Live Next.js dashboard deployed on Vercel → https://ecommerce-return-gatekeeper.vercel.app
- [x] Triage Router Demo page functional
- [x] Public GitHub repository → https://github.com/ysranis/ecommerce-return-gatekeeper
- [ ] Qwen model card on Hugging Face Hub *(pending — RunPod pod terminated before upload; will upload after Run 2)*
- [ ] Llama model card on Hugging Face Hub *(pending — same reason)*
- [x] README complete with results narrative and architecture
- [x] **Quality Gate 4 passed:** Dashboard clearly shows cost-performance tradeoffs across all 5 models

### Run 2 — In Progress (branch: feature/run2-training)

#### Diagnosis (completed 2026-07-15)

Post-Run-1 analysis of the confusion matrices identified three concrete root causes for the gatekeeper accuracy gap:

**Root Cause 1 — CLASS IMBALANCE: `REQUEST_EVIDENCE` is starved**

| Class | Train | Val | Test |
|---|---|---|---|
| APPROVE_AUTOMATED | 477 (39.8%) | 70 (46.7%) | 63 (42.0%) |
| ESCALATE_TO_HUMAN | 579 (48.2%) | 63 (42.0%) | 66 (44.0%) |
| REQUEST_EVIDENCE | 144 (12.0%) | 17 (11.3%) | 21 (14.0%) |

Only 12% of training examples are REQUEST_EVIDENCE. The model barely learned the decision boundary for this class.

**Root Cause 2 — PER-CLASS ACCURACY COLLAPSE on `REQUEST_EVIDENCE`**

| Class | FT Qwen per-class acc. | FT Llama per-class acc. |
|---|---|---|
| APPROVE_AUTOMATED | 66.7% (42/63) | 60.3% (38/63) |
| ESCALATE_TO_HUMAN | 69.7% (46/66) | 54.5% (36/66) |
| REQUEST_EVIDENCE | **23.8% (5/21)** | 61.9% (13/21) |

FT Qwen gets REQUEST_EVIDENCE right only 1-in-4 times. When wrong, it collapses to ESCALATE_TO_HUMAN (15/16 errors → ESCALATE). Both classes mean "not automated", so the model conflates them when training signal for REQUEST_EVIDENCE is weak.

**Root Cause 3 — BLIND SPOT: `cancel_order` + `REQUEST_EVIDENCE` = 0 training examples**

| Intent | APPROVE | ESCALATE | REQUEST_EVIDENCE |
|---|---|---|---|
| cancel_order | 360 | 56 | **0** |
| get_refund | 66 | 421 | 128 |

Of 416 cancel_order examples, none have REQUEST_EVIDENCE. The model has never seen "cancel but evidence required first." It also over-associates cancel_order with APPROVE_AUTOMATED (360/416 = 86.5%).

**Prompt Alignment:** No mismatch. Training and eval both use identical `LABELING_SYSTEM_PROMPT` with `apply_chat_template`. Issue is purely data.

---

#### Remediation Plan

| Fix | Target | Rationale |
|---|---|---|
| Upsample REQUEST_EVIDENCE to ~25% of train | +~160 new synthetic rows | Fixes class starvation; target distribution: APPROVE 35%, ESCALATE 40%, REQUEST_EVIDENCE 25% |
| Add cancel_order + REQUEST_EVIDENCE examples | ~30 new rows | Closes zero-shot blind spot where cancel = always approve |
| Sharpen chain_of_thought labels | Applied to all new rows | New examples explicitly state the REQUEST_EVIDENCE trigger in CoT: "item damaged → photo required → REQUEST_EVIDENCE, not ESCALATE" |
| Increase Qwen LoRA rank r=16→32 | Track A only | Qwen showed worse REQUEST_EVIDENCE accuracy (23.8%) — more adapter capacity needed |
| Extend Qwen training to 4 epochs | Track A only | Qwen has larger capacity; extra epoch helps consolidate the new class signal |

---

#### Script Additions for Run 2

- `scripts/09_generate_request_evidence.py` — Targeted synthetic generation for REQUEST_EVIDENCE rows. Generates ~200 new rows across all intent categories with REQUEST_EVIDENCE gatekeeper status. Outputs to `data/request_evidence_supplement.jsonl`, then merges into a new `data/train_v2.jsonl`.

---

#### Run 2 Deliverables Checklist

- [ ] `scripts/09_generate_request_evidence.py` written ✅
- [ ] `data/request_evidence_supplement.jsonl` generated (~200 rows)
- [ ] `data/train_v2.jsonl` merged dataset (~1,360 rows, ~25% REQUEST_EVIDENCE)
- [ ] `scripts/05_train_qwen.py` updated — r=32, 4 epochs, point to train_v2.jsonl
- [ ] `scripts/06_train_llama.py` updated — point to train_v2.jsonl
- [ ] Run 2 training on RunPod (Track A + Track B)
- [ ] `results/finetuned_results_qwen_v2.json` — Run 2 Qwen eval
- [ ] `results/finetuned_results_llama_v2.json` — Run 2 Llama eval
- [ ] `results/eval_summary.json` updated with Run 2 results
- [ ] Dashboard DB re-seeded with Run 2 results
- [ ] Qwen model card uploaded to Hugging Face Hub
- [ ] Llama model card uploaded to Hugging Face Hub

#### Run 2 RunPod Execution Order

```bash
# 1. Generate supplement data (run locally before RunPod)
python scripts/09_generate_request_evidence.py --n-rows 200 --output-dir data/

# 2. RunPod — Train Track A (Qwen, r=32, 4 epochs, train_v2.jsonl)
python scripts/05_train_qwen.py

# 3. RunPod — Train Track B (Llama, r=8, 3 epochs, train_v2.jsonl)
python scripts/06_train_llama.py

# 4. RunPod — Eval Run 2 fine-tuned models
...
# 5. Locally — compare Run 1 vs Run 2
python scripts/04_compare_results.py
```

#### Run 2 Updated Training Config

**Track A — Qwen-2.5-7B (updated):**
- model: `Qwen/Qwen2.5-7B-Instruct`, `load_in_4bit=True`
- LoRA: `r=32` *(was 16)*, `lora_alpha=64` *(was 32)*, `dropout=0.05`, target=`["q_proj","v_proj"]`
- Training: `epochs=4` *(was 3)*, `batch=4`, `grad_accum=4`, `lr=2e-4`, cosine schedule, bf16
- Dataset: `data/train_v2.jsonl` *(was train.jsonl)*
- Output: `output/qwen-2.5-7b-ecommerce-gk-v2`
- W&B: `project=ecommerce-gatekeeper`, `name=qwen-7b-run-002`, `group=qwen-7b`

**Track B — Llama-3.2-3B (unchanged except dataset):**
- model: `meta-llama/Llama-3.2-3B-Instruct`, `load_in_4bit=True`
- LoRA: `r=8`, `lora_alpha=16`, `dropout=0.05`, target=`["q_proj","v_proj"]`
- Training: `epochs=3`, `batch=8`, `grad_accum=2`, `lr=2e-4`, cosine schedule, bf16
- Dataset: `data/train_v2.jsonl` *(was train.jsonl)*
- Output: `output/llama-3.2-3b-ecommerce-gk-v2`
- W&B: `project=ecommerce-gatekeeper`, `name=llama-3b-run-002`, `group=llama-3b`

#### Run 2 Quality Gate

Gatekeeper accuracy must clear the following thresholds to declare Run 2 successful:

| Model | Run 1 | Run 2 Target |
|---|---|---|
| FT Qwen-2.5-7B | 62.0% | ≥75% |
| FT Llama-3.2-3B | 58.0% | ≥70% |
| REQUEST_EVIDENCE per-class (Qwen) | 23.8% | ≥55% |
| REQUEST_EVIDENCE per-class (Llama) | 61.9% | ≥70% |

*(PRD targets of ≥97%/≥92% are aspirational; Run 2 targets reflect realistic gains from data fixes alone.)*

---

#### What you now have — the complete project (Run 1 baseline)

You have a fully working, publicly deployed AI system with:
- A curated synthetic training dataset (built with a frontier AI)
- Two fine-tuned open-source models (adapter weights on RunPod)
- A rigorous multi-model evaluation with LLM-as-a-Judge and statistical significance
- A live web dashboard showing cost-performance tradeoffs
- A clean public GitHub repository with everything documented
