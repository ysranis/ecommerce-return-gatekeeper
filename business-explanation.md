...
| Tool | What It Is | Why We Use It |
|---|---|---|
| **Custom Python harness** | `scripts/07_evaluate_models.py` | Automated scoring: JSON validity, intent accuracy, gatekeeper accuracy, slot F1, hallucination rate |
| **DeepSeek-V3 (as judge)** | Used as the LLM-as-a-Judge model | Rates each of 750 model outputs 1–5 vs. ground truth; cheapest option (~$0.11) |
| **Bootstrap resampling** | 1,000 resamples per metric | Produces 95% confidence intervals to confirm improvements are statistically significant |
...
### Actual Project Costs (Run 1 — Completed)

| Cost Item | Actual Cost | Notes |
|---|---|---|
| DeepSeek-V3 API — Dataset Generation | ~$0.90 | 2,000 seed message rewriting calls |
| DeepSeek-V3 API — Golden Labeling | ~$1.85 | 2,000 structured JSON label generation calls |
| GPU Rental — Both models training + baseline eval | ~$1.50 | RunPod RTX 4090, single session (~3–4 hours) |
| DeepSeek-V3 API — 5-model benchmark teacher eval | ~$0.10 | ~150 inference calls for DeepSeek-V3 as 5th model |
| DeepSeek-V3 API — LLM Judge (750 calls) | ~$0.11 | Judge scoring all 5 models × 150 test rows |
| Vercel (Dashboard hosting) | Free | Vercel Hobby plan |
| Neon Postgres | Free | Free tier via Vercel Storage |
...
| **Total Actual Project Cost** | **~$5–6** | End-to-end: dataset + training + eval + dashboard |

> The original budget estimate of $20–35 was conservative. Actual cost came in at ~$5–6 due to: (1) smaller total API token usage than estimated, (2) using a RTX 4090 instance efficiently in one session rather than spread across multiple A10G sessions, and (3) using DeepSeek-V3 (cheapest) rather than Claude Haiku as the LLM judge.
...
> *"After fine-tuning, the 7B Qwen model's intent accuracy jumped from 61% to 86% — a +25 percentage point improvement. The 3B Llama model improved even more dramatically, from 24% to 74% intent accuracy — +50 percentage points. Both models showed strong LLM judge score improvements: Qwen from 3.1 to 3.9 out of 5, Llama from 2.4 to 3.7. I benchmarked all five model variants using automated metrics and an LLM-as-a-Judge framework with bootstrap confidence intervals. A second training run is underway to push gatekeeper accuracy higher."*
...
> *"Built and shipped an end-to-end model distillation and fine-tuning benchmark for e-commerce dispute arbitration — +50pp intent accuracy improvement on Llama-3B, +25pp on Qwen-7B, 97% inference cost reduction vs. GPT-4o, with a live analytics dashboard benchmarking all 5 model variants."*
...
• Designed and executed an end-to-end model distillation pipeline using DeepSeek-V3 as a
  ...
  at ~$5 total project cost

• Fine-tuned Qwen-2.5-7B and Llama-3.2-3B using QLoRA (LoRA r=16/8, cosine LR schedule,
  3 epochs): intent accuracy improved +24.7pp (Qwen: 61%→86%) and +50pp (Llama: 24%→74%);
  LLM judge scores improved from 3.1→3.9 and 2.4→3.7 (out of 5)
...
• Built a 5-model evaluation harness with LLM-as-a-Judge scoring (DeepSeek-V3 as judge),
  bootstrap 95% confidence intervals across 750 outputs (5 models × 150 held-out test rows)

• Shipped a Next.js 14 analytics dashboard (Vercel + Neon Postgres) with live model
  comparison, row-level inspection, interactive cost calculator, and triage router demo
  — live at ecommerce-return-gatekeeper.vercel.app
```
